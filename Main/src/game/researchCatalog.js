...
export function getResearchIconNode(nodeOrIdRaw) {
  let node = (nodeOrIdRaw && typeof nodeOrIdRaw === "object") ? nodeOrIdRaw : getResearchNode(nodeOrIdRaw);
  const seen = new Set();
  while (node) {
    ...
    if (!requires.length) return node;
    const parent = getResearchNode(requires[0]);
...
export function getResearchIconCandidates(nodeOrNameRaw) {
  const node = (nodeOrNameRaw && typeof nodeOrNameRaw === "object") ? nodeOrNameRaw : null;
  const rawId = String(node?.id || "").trim();
  const rawName = String(node?.name || (!node ? nodeOrNameRaw : "") || "").trim();
  if (!rawId && !rawName) return [];
  const nameSource = rawName || rawId;
  const compact = nameSource.replace(/\s+/g, " ").trim();
