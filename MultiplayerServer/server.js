...
const MB = 1024 * 1024;
const HOST_CPU_COUNT = Math.max(
  1,
  Number((typeof os.availableParallelism === "function" ? os.availableParallelism() : (Array.isArray(os.cpus()) ? os.cpus().length : 1)) || 1) | 0
);
const HOST_TOTAL_MEM_MB = Math.max(
  256,
  Math.round((Number(typeof os.totalmem === "function" ? os.totalmem() : 0) || 0) / MB)
);
const HIGH_CAPACITY_MULTIPLAYER_HOST = HOST_CPU_COUNT >= 8 && HOST_TOTAL_MEM_MB >= 7168;
const MATCH_SNAPSHOT_INTERVAL_MS = Math.max(28, Number(process.env.MATCH_SNAPSHOT_INTERVAL_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 34 : 42)));
const MATCH_SNAPSHOT_INTERVAL_MIN_MS = Math.max(24, Number(process.env.MATCH_SNAPSHOT_INTERVAL_MIN_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 28 : 34)));
const MATCH_SNAPSHOT_INTERVAL_MAX_MS = Math.max(
  MATCH_SNAPSHOT_INTERVAL_MIN_MS,
  Number(process.env.MATCH_SNAPSHOT_INTERVAL_MAX_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 92 : 120))
);
...
const MATCH_SNAPSHOT_FORCE_INTERVAL_MS = Math.max(90, Number(process.env.MATCH_SNAPSHOT_FORCE_INTERVAL_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 120 : 150)));
...
const MATCH_TILE_DELTA_CAP = Math.max(1000, Number(process.env.MATCH_TILE_DELTA_CAP || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 9500 : 7000)));
const MATCH_TILE_DELTA_DRAIN_MIN = Math.max(500, Number(process.env.MATCH_TILE_DELTA_DRAIN_MIN || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 1100 : 700)));
const MATCH_TILE_DELTA_BACKLOG_CAP = Math.max(MATCH_TILE_DELTA_CAP, Number(process.env.MATCH_TILE_DELTA_BACKLOG_CAP || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 260000 : 180000)));
const MATCH_TERRITORY_PULSE_INTERVAL_MS = Math.max(14, Number(process.env.MATCH_TERRITORY_PULSE_INTERVAL_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 18 : 24)));
const MATCH_TERRITORY_PULSE_INTERVAL_MAX_MS = Math.max(
  MATCH_TERRITORY_PULSE_INTERVAL_MS,
  Number(process.env.MATCH_TERRITORY_PULSE_INTERVAL_MAX_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 64 : 84))
);
const MATCH_TERRITORY_PULSE_CAP = Math.max(200, Number(process.env.MATCH_TERRITORY_PULSE_CAP || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 3200 : 2200)));
const MATCH_OWNER_SWEEP_CHUNK_MIN = Math.max(300, Number(process.env.MATCH_OWNER_SWEEP_CHUNK_MIN || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 950 : 700)));
const MATCH_OWNER_SWEEP_CHUNK_MAX = Math.max(MATCH_OWNER_SWEEP_CHUNK_MIN, Number(process.env.MATCH_OWNER_SWEEP_CHUNK_MAX || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 3600 : 2600)));
...
const MATCH_ENTITY_DELTA_INTERVAL_MS = Math.max(34, Number(process.env.MATCH_ENTITY_DELTA_INTERVAL_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 46 : 56)));
const MATCH_ENTITY_DELTA_INTERVAL_MAX_MS = Math.max(
  MATCH_ENTITY_DELTA_INTERVAL_MS,
  Number(process.env.MATCH_ENTITY_DELTA_INTERVAL_MAX_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 220 : 280))
);
const MATCH_STRUCTURE_DELTA_INTERVAL_MS = Math.max(90, Number(process.env.MATCH_STRUCTURE_DELTA_INTERVAL_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 240 : 320)));
const MATCH_STRUCTURE_DELTA_INTERVAL_MAX_MS = Math.max(
  MATCH_STRUCTURE_DELTA_INTERVAL_MS,
  Number(process.env.MATCH_STRUCTURE_DELTA_INTERVAL_MAX_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 820 : 1100))
);
const MATCH_OPERATIONS_DELTA_INTERVAL_MS = Math.max(30, Number(process.env.MATCH_OPERATIONS_DELTA_INTERVAL_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 44 : 52)));
const MATCH_MOBILE_DELTA_INTERVAL_MS = Math.max(24, Number(process.env.MATCH_MOBILE_DELTA_INTERVAL_MS || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 38 : 46)));
const MATCH_BACKPRESSURE_SOFT_BYTES = Math.max(64 * 1024, Number(process.env.MATCH_BACKPRESSURE_SOFT_BYTES || (HIGH_CAPACITY_MULTIPLAYER_HOST ? (512 * 1024) : (384 * 1024))));
const MATCH_BACKPRESSURE_HARD_BYTES = Math.max(MATCH_BACKPRESSURE_SOFT_BYTES, Number(process.env.MATCH_BACKPRESSURE_HARD_BYTES || (HIGH_CAPACITY_MULTIPLAYER_HOST ? (3 * 1024 * 1024) : (2 * 1024 * 1024))));
...
const MATCH_BACKPRESSURE_RECOVERY_SYNC_BUFFER_BYTES = Math.max(
  ...
  Math.min(
    ...
    Number(process.env.MATCH_BACKPRESSURE_RECOVERY_SYNC_BUFFER_BYTES || Math.round(MATCH_BACKPRESSURE_SOFT_BYTES * (HIGH_CAPACITY_MULTIPLAYER_HOST ? 0.50 : 0.42)))
  )
);
...
const MATCH_WIRE_SOFT_WORLD_TILES = Math.max(400000, Number(process.env.MATCH_WIRE_SOFT_WORLD_TILES || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 2800000 : 2200000)));
const MATCH_WIRE_SOFT_AI_COUNT = Math.max(8, Number(process.env.MATCH_WIRE_SOFT_AI_COUNT || (HIGH_CAPACITY_MULTIPLAYER_HOST ? 120 : 90)));
