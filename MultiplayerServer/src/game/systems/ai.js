  ...
  World.prototype._aiStrategize = function(id) {
      ...
      if (wars > 0) {
        ...
        if (
          ...
          if (wars >= 2) askP += 0.08;
          if (underThreat) askP += 0.08 * coalition;
          if (!allyPick) {
            ai.allianceCooldownUntil = this.time + 12 + this._rng() * 18;
          } else if (this._rng() < clamp01(askP)) {
            allianceAttempted = true;
            const req = this.requestAlliance(A, allyPick);
            ai.allianceCooldownUntil = this.time + (req.ok ? (42 + this._rng() * 50) : (22 + this._rng() * 30));
      ...
      if (
        ...
        let askP = 0.03 + 0.15 * diplomacy;
        if (coalitionMoment) askP += (0.08 + 0.13 * runawayPressure) * coalition;
        if (underThreat) askP += 0.09;
        if (!allyPick) {
          ai.allianceCooldownUntil = this.time + 12 + this._rng() * 18;
        } else if (this._rng() < clamp01(askP)) {
          allianceAttempted = true;
          const req = this.requestAlliance(A, allyPick);
          ai.allianceCooldownUntil = this.time + (req.ok ? (40 + this._rng() * 46) : (20 + this._rng() * 28));
          if (req.ok) {
      ...
      if (
        ...
        allyCount === 0 &&
        canSeekAlliance &&
        this._rng() < (0.008 + 0.03 * diplomacy)
      ) {
        const allyPick = this._aiPickAllianceCandidate(A, 0, p, myStr);
        if (allyPick) {
          allianceAttempted = true;
          const req = this.requestAlliance(A, allyPick);
          ai.allianceCooldownUntil = this.time + (req.ok ? (44 + this._rng() * 50) : (22 + this._rng() * 26));
          if (req.ok) ai.lastAllianceTarget = allyPick;
        } else {
          ai.allianceCooldownUntil = this.time + 12 + this._rng() * 16;
        }
      }
    }
